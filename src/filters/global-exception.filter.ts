...
export class GlobalExceptionFilter implements ExceptionFilter {
  ...
  private handleUnprocessableEntityException(
    ...
    const errorRes = {
      ...
      statusCode,
      error: STATUS_CODES[statusCode] || 'Unprocessable Entity', // Fallback value
  ...
  private handleValidationException(exception: ValidationException): ErrorDto {
    ...
    const errorRes = {
      timestamp: new Date().toISOString(),
      statusCode,
      error: STATUS_CODES[statusCode] || 'Bad Request', // Fallback value
      errorCode:
  ...
  private handleHttpException(exception: HttpException): ErrorDto {
    ...
    const errorRes = {
      timestamp: new Date().toISOString(),
      statusCode,
      error: STATUS_CODES[statusCode] || 'Error', // Fallback value
  ...
  private handleError(error: Error): ErrorDto {
    ...
    const errorRes = {
      timestamp: new Date().toISOString(),
      statusCode,
      error: STATUS_CODES[statusCode] || 'Internal Server Error', // Fallback value
